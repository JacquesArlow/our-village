...
// Button 1 — compose a WhatsApp message to the practice.
...
// Button 2 — email the enquiry to reception. Opens the visitor's mail client
// pre-addressed and pre-filled, so it lands in reception@our-village.co.za.
function emailReception() {
  const subject = form.subject || `Website enquiry — ${form.firstName} ${form.lastName}`.trim()
...
<template>
  <form class="space-y-4" @submit.prevent="sendWhatsApp">
    ...
    <div class="flex flex-col gap-3 pt-1 sm:flex-row">
      <button type="submit" class="btn btn-primary flex-1 gap-2 rounded-full font-semibold normal-case">
      ...
      <button type="button" class="btn btn-outline btn-primary flex-1 gap-2 rounded-full font-semibold normal-case" @click="emailReception">
    ...
    <p class="text-xs text-base-content/55">
      “Send via WhatsApp” opens a chat with us; “Email reception” opens your mail app addressed to
